...
- (void)startLoading{
    ...
    if (![mutableRequest.allHTTPHeaderFields.allKeys containsObject:HEADER_X_API_UA]) {
        NSMutableURLRequest *mutableRequest = [request mutableCopy];
        NSDictionary *dict = [AFNetworking_RequestData createHeaderDict];
        if (dict != nil) {
            NSArray *keyArr = [dict allKeys];
            for (NSString *key in keyArr) {
                [mutableRequest addValue:[dict objectForKey:key] forHTTPHeaderField:key];
            }
        }
    }
